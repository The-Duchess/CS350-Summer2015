...
lines.each do |line|
      ...
      p t_tokens.map(&:to_s)
      p t_tokens.map(&:to_s).map { |start_p, end_p, length| { :start_p => start_p, :end_p => end_p, :length => length } }
      t_edge = t_tokens.map(&:to_s).map { |start_p, end_p, length| { :start_p => start_p, :end_p => end_p, :length => length } }
      edges.push(t_edge)
end
